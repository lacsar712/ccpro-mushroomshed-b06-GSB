...
DUPLICATE_DETAIL = "该出菇室在此时间已登记过色斑备忘"
...
@jwt_required()
def delete_color_note(note_id: int):
    db = SessionLocal()
    try:
        item = db.query(ColorNote).filter(ColorNote.id == note_id).first()
        if not item:
            return jsonify({"detail": "色斑备忘不存在"}), 404
        db.delete(item)
        db.commit()
        return "", 204
    finally:
